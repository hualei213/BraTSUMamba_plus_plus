...
from mamba_AdM.Slice_Hilbert_Interleave import Mamba
...
class PlainConvUNet(nn.Module):
    def __init__(
        ...
        self.template_encoder = self.encoder[0]

        # --- decoder (same as your v2, with tri-plane frequency fusion etc.) ---
        self.decoder = UNetDecoder(
            self.template_encoder, num_classes, n_conv_per_stage_decoder, deep_supervision,
    ...
    def forward(self, x):
        ...
        all_skips = [enc(xm) for enc, xm in zip(self.encoder, xs)]  # list[M] of list[S]
