    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "Bi":
                ...
                # out = F.linear(rearrange(out + out_b.flip([-1]) + out_s+out_s_b.flip([-1]), "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
                out_sq = F.linear(rearrange(out + out_b.flip([-1]), "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
                out_slice = F.linear(rearrange(out_s+out_s_b.flip([-1]), "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
