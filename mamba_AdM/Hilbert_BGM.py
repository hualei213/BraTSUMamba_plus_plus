...
def _hilbert_idx_2d_linear(N: int) -> torch.Tensor:
    ...
    idx = [0] * L
    for t in range(L):
        x, y = _hilbert_i2c(2, p, t)  # x: w, y: h
        idx[t] = y * N + x
...
def _hilbert_idx_3d_linear(N: int) -> torch.Tensor:
    ...
    idx = [0] * L
    for k in range(L):
        x, y, z = _hilbert_i2c(3, p, k)  # x: w, y: h, z: d
        idx[k] = z * (N * N) + y * N + x
    return torch.tensor(idx, dtype=torch.long)
...
def _slice_hilbert_interleave_idx(N: int) -> torch.Tensor:
    """
    [Optimized] 向量化版本，移除 for 循环以消除 Graph Break 警告
    """
    if not _is_pow2(N):
        raise ValueError(f"Slice-Hilbert-Interleave requires N be power-of-two, got N={N}")

    # 1. 获取 2D 希尔伯特索引 (Shape: [N^2])
    plane = _hilbert_idx_2d_linear(N)

    # 2. 构造深度偏移 (Shape: [N])
    # 对应原来的 d * base_stride
    base_stride = N * N
    depth_offsets = torch.arange(N, device=plane.device) * base_stride
