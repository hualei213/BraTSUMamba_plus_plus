...
class nnUNetPredictor(object):
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                try:
                    ...
                    original_shape = props['shape_before_cropping']
                    bbox = props['bbox_used_for_cropping']
                    shape_before_resampling = props['shape_after_cropping_and_before_resampling']

                    # 2. 准备保存路径
                    fname = os.path.basename(preprocessed['ofile']) if preprocessed['ofile'] else "unknown_case"
                    case_id = fname.replace('.nii.gz', '')

                    save_dir_full = "/mnt/HDLV1/YHR/nnUNet-BraTSUMamba/nnUNet_results/Dataset1003_MSD/low_frequency_maps_fullres"
                    os.makedirs(save_dir_full, exist_ok=True)
                    target_size = tuple(shape_before_resampling)
                    ...
                    # 定义内部函数：执行逆变换 (Resample back + Uncrop)
                    def restore_to_original_geometry(tensor_data, is_prediction=True):
                        # 3.1 逆重采样 (Resample)
                        # tensor_data shape: (C, D, H, W)
                        # 转换为 (1, C, D, H, W) 进行插值
                        if isinstance(tensor_data, np.ndarray):
                            tensor_data = torch.from_numpy(tensor_data)

                        batch_tensor = tensor_data.unsqueeze(0).float()

                        # 预测结果通常用 trilinear，输入图像也可以用 trilinear (或者 bilinear)
                        # 这里统一用 trilinear 保证平滑
                        resampled = F.interpolate(batch_tensor, size=target_size,
                                                  mode='trilinear', align_corners=False)
                        resampled_data = resampled[0]  # (C, D', H', W')

                        # 3.2 逆裁剪 (Uncrop / Paste)
                        # 创建全0画布 (C, Orig_D, Orig_H, Orig_W)
                        full_vol = torch.zeros((tensor_data.shape[0], *original_shape), dtype=torch.float32)

                        # 计算实际贴图尺寸 (处理边界舍入误差)
                        d_len = min(z_e - z_s, resampled_data.shape[1])
                        h_len = min(y_e - y_s, resampled_data.shape[2])
                        w_len = min(x_e - x_s, resampled_data.shape[3])

                        # 贴回去
                        full_vol[:, z_s:z_s + d_len, y_s:y_s + h_len, x_s:x_s + w_len] = \
                            resampled_data[:, :d_len, :h_len, :w_len]

                        return full_vol.numpy()

                    # === A. 处理 Feature (Prediction) ===
                    final_full_feat = restore_to_original_geometry(prediction)
                    save_path_feat = os.path.join(save_dir_full, f"feature_full_{case_id}.npy")
                    np.save(save_path_feat, final_full_feat)
                    print(f"✅ [Feature] 已还原保存: {save_path_feat} | {final_full_feat.shape}")

                    # === B. 处理 Input (Image) ===
                    # data 是预处理后的输入 tensor (C, D, H, W)
                    # 注意：data 可能还在 GPU 上，或者已经被转为 numpy
                    if isinstance(data, torch.Tensor):
                        data_cpu = data.cpu().detach()
                    else:
                        data_cpu = torch.from_numpy(data)

                    final_full_input = restore_to_original_geometry(data_cpu)
                    save_path_input = os.path.join(save_dir_full, f"input_full_{case_id}.npy")
                    np.save(save_path_input, final_full_input)
                    print(f"✅ [Input]   已还原保存: {save_path_input} | {final_full_input.shape}")
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        # # 定义保存目录
        # save_dir = "/mnt/HDLV1/YHR/nnUNet-BraTSUMamba/nnUNet_results/Dataset1003_MSD/frequency_maps"
        # os.makedirs(save_dir, exist_ok=True)
        #
        # # 转为 Numpy (Channel, Depth, Height, Width)
        # high_freq_vol = prediction.cpu().numpy()
        #
        # # 生成带时间戳的文件名
        # timestamp = str(time.time()).replace('.', '')[-6:]
        # save_path = os.path.join(save_dir, f"high_freq_4ch_{timestamp}.npy")
        #
        # # 保存！
        # np.save(save_path, high_freq_vol)
        # print(f"🔥🔥🔥 完整拼接的4通道高频特征已保存: {save_path}")
        # print(f"    数据形状: {high_freq_vol.shape}")
        # # ===============================================

        # # 1. 定义保存路径
        # save_dir = "/mnt/HDLV1/YHR/nnUNet-BraTSUMamba/nnUNet_results/Dataset1003_MSD/low_frequency_maps"
        # os.makedirs(save_dir, exist_ok=True)
        # timestamp = str(time.time()).replace('.', '')[-6:]
        #
        # # 2. 保存高频特征 (Output)
        # high_freq_vol = prediction.cpu().numpy()
        # save_path_out = os.path.join(save_dir, f"feature_{timestamp}.npy")
        # np.save(save_path_out, high_freq_vol)
        #
        # # 3. 保存输入图像 (Input) - 关键步骤！
        # # data 是 Tensor，形状也是 (4, D, H, W)，对应 4 个模态
        # input_vol = data.cpu().numpy()
        # save_path_in = os.path.join(save_dir, f"input_{timestamp}.npy")
        # np.save(save_path_in, input_vol)
        #
        # print(f"🔥🔥🔥 已保存配对数据:")
        # print(f"    输入(Input):   {save_path_in}")
        # print(f"    特征(Feature): {save_path_out}")
        # ===============================================
...
if __name__ == '__main__':
    ...
    # from BraTSUMamba.paths import nnUNet_results, nnUNet_raw
    #
    ...
    # # from BraTSUMamba.imageio.simpleitk_reader_writer import SimpleITKIO
    # #
