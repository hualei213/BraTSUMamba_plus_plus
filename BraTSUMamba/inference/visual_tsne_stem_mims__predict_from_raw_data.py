...
import BraTSUMamba
from BraTSUMamba.configuration import default_num_processes
from BraTSUMamba.inference.data_iterators import PreprocessAdapterFromNpy, preprocessing_iterator_fromfiles, \
    preprocessing_iterator_fromnpy
from BraTSUMamba.inference.export_prediction import export_prediction_from_logits, \
    convert_predicted_logits_to_segmentation_with_correct_shape
from BraTSUMamba.inference.sliding_window_prediction import compute_gaussian, \
    compute_steps_for_sliding_window
from BraTSUMamba.utilities.file_path_utilities import get_output_folder, check_workers_alive_and_busy
from BraTSUMamba.utilities.find_class_by_name import recursive_find_python_class
from BraTSUMamba.utilities.helpers import empty_cache, dummy_context
from BraTSUMamba.utilities.json_export import recursive_fix_for_json_export
from BraTSUMamba.utilities.label_handling.label_handling import determine_num_input_channels
from BraTSUMamba.utilities.plans_handling.plans_handler import PlansManager, ConfigurationManager
from BraTSUMamba.utilities.utils import create_lists_from_splitted_dataset_folder
...
# ==========================================
# [ADDITION FOR T-SNE] Helper Classes
# ==========================================
# ==========================================
# [TSNE] Helper Classes
# ==========================================
class FeatureHook:
    def __init__(self, module, capture_input=False):
        self.hook = None
        self.feature = None

        if capture_input:
            # 使用 Pre-hook 捕获“输入”
            self.hook = module.register_forward_pre_hook(self.pre_hook_fn)
        else:
            # 使用 Standard hook 捕获“输出”
    ...
    def pre_hook_fn(self, module, input):
        # Pre-hook 的 input 是一个 tuple
        self.feature = input[0].detach().cpu()
...
def apply_center_crop(tensor, target_shape=(128, 128, 128)):
    """Center crop tensor to target shape to match network patch size"""
    if tensor.ndim == 4:  # (C, D, H, W)
        _, D, H, W = tensor.shape
        sd = max(0, (D - target_shape[0]) // 2)
        sh = max(0, (H - target_shape[1]) // 2)
        sw = max(0, (W - target_shape[2]) // 2)
        return tensor[:, sd:sd + target_shape[0], sh:sh + target_shape[1], sw:sw + target_shape[2]]
...
def sample_features(feature_map, mask_data, num_samples=2000):
    """
    Sample features based on mask labels.
    使用 MaxPool 进行下采样，确保肿瘤标签不丢失。
    """
    C, Df, Hf, Wf = feature_map.shape
    D_orig, H_orig, W_orig = mask_data.shape

    mask_tensor = torch.from_numpy(mask_data).float().unsqueeze(0).unsqueeze(0)  # (1, 1, D, H, W)
    ...
    # [关键修改] 使用 MaxPool3d 而不是 interpolate
    # 只要局部区域有肿瘤(label>0)，就保留最大值(即保留肿瘤标签)
    if stride_d >= 1 and stride_h >= 1 and stride_w >= 1:
        # 确保 kernel_size 不会小于 1
        k_d, k_h, k_w = max(1, stride_d), max(1, stride_h), max(1, stride_w)
        try:
            mask_resized = torch.nn.functional.max_pool3d(
                mask_tensor,
                kernel_size=(k_d, k_h, k_w),
                stride=(k_d, k_h, k_w)
            )
            # 极少数情况下尺寸可能稍微不匹配(由于整除)，强制插值对齐一下
            if mask_resized.shape[2:] != (Df, Hf, Wf):
                mask_resized = torch.nn.functional.interpolate(mask_resized, size=(Df, Hf, Wf), mode='nearest')
        except Exception as e:
            print(f"MaxPool failed: {e}, falling back to nearest")
            mask_resized = torch.nn.functional.interpolate(mask_tensor, size=(Df, Hf, Wf), mode='nearest')
    else:
        # 如果是上采样阶段 (decoder)，用 nearest
        mask_resized = torch.nn.functional.interpolate(mask_tensor, size=(Df, Hf, Wf), mode='nearest')

    labels_flat = mask_resized.squeeze().long().reshape(-1)
    features_flat = feature_map.permute(1, 2, 3, 0).reshape(-1, C)

    # 打印调试信息：看看下采样后还剩多少肿瘤点
    unique_labels = torch.unique(labels_flat)
    # print(f"  [DEBUG] Labels after downsampling to {Df}x{Hf}x{Wf}: {unique_labels.tolist()}")

    # BraTS Labels mapping
    label_dict = {0: 'Background', 1: 'NCR', 2: 'ED', 3: 'ET'}
    ...
    for lbl in unique_labels:
        ...
        # 采样策略：背景少采一点，肿瘤多采一点
        current_n = num_samples if lbl_idx != 0 else num_samples // 4
    ...
    if not sampled_X: return None, None
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        trainer_class = recursive_find_python_class(join(BraTSUMamba.__path__[0], "training", "nnUNetTrainer"),
                                                    trainer_name, 'BraTSUMamba.training.nnUNetTrainer')
        if trainer_class is None:
            raise RuntimeError(f'Unable to locate trainer class {trainer_name} in BraTSUMamba.training.nnUNetTrainer. '
    ...
    # ... inside nnUNetPredictor class ...
    # [ADDITION FOR T-SNE] New Method
        # [UPDATED FOR T-SNE] 修复版：使用预测结果染色 + 智能采样
    def predict_and_plot_tsne(self, input_folder, output_folder, target_layer='encoder.stages.5',
                              max_cases=3, num_samples=5000):
        """
        Run inference, extract features, and plot T-SNE using PREDICTED labels.
        """
        import random
        from BraTSUMamba.utilities.file_path_utilities import check_workers_alive_and_busy

        # 1. Setup Data List
        file_ending = self.dataset_json['file_ending']
        all_files = [f for f in os.listdir(input_folder) if f.endswith(file_ending) and "_0000" in f]
        case_ids = sorted(list(set([f.split("_0000")[0] for f in all_files])))

        if len(case_ids) > max_cases:
            case_ids = random.sample(case_ids, max_cases)
            print(f"[T-SNE] Randomly selected cases: {case_ids}")
        ...
        for name, module in self.network.named_modules():
            if name == target_layer:
                # [关键修改] 如果目标层是 up_sample，我们需要抓取它的“输入”（即上采样前的特征）
                # 依据：mims_v2.py 中 self.up_sample(y)，y 是上采样前的特征
                should_capture_input = (target_layer == 'stem_mims.up_sample')

                # 实例化 Hook，根据判断决定抓取 Input 还是 Output
                hook_handle = FeatureHook(module, capture_input=should_capture_input)

                if should_capture_input:
                    print(f"[T-SNE] Hook registered on: {name} (Mode: Capture INPUT / Pre-Upsample)")
                else:
                    print(f"[T-SNE] Hook registered on: {name} (Mode: Capture OUTPUT)")
                break
        ...
        patch_size = self.configuration_manager.patch_size  # [128, 128, 128]

        for case_id in tqdm(case_ids, desc="Extracting Features"):
            # Prepare file lists
            data_files = [join(input_folder, f"{case_id}_{i:04d}{file_ending}")
                          for i in range(len(self.dataset_json['channel_names']))]

            # Preprocess
            # 注意：即使有 labelsTs，我们也主要依赖预测结果，防止裁切导致的标签丢失
            data, _, _ = preprocessor.run_case(data_files, None, self.plans_manager,
                                               self.configuration_manager, self.dataset_json)

            # Prepare Tensor & CROP
            data_tensor = torch.from_numpy(data).unsqueeze(0)  # (1, C, D, H, W)
            data_tensor = apply_center_crop(data_tensor, patch_size)
            data_tensor = data_tensor.to(self.device)

            # Forward Pass (获取模型输出!)
            with torch.no_grad():
                output = self.network(data_tensor)  # Output: (1, NumClasses, D, H, W)

            # --- [新增保护] 检查 Hook 是否捕获到了数据 ---
            if hook_handle.feature is None:
                print(f"[Warning] Layer '{target_layer}' was not executed in forward pass. Skipping case.")
                continue  # 跳过当前病例
            # --------------------------------------------
            ...
            # --- [CRITICAL CHANGE] Use Model Prediction as Label ---
            # 获取预测的 Segmentation Map
            if isinstance(output, tuple): output = output[0]  # Handle deep supervision

            # 这里的 output 是 logits，我们需要 argmax 得到类别 (0,1,2,3)
            # 同时也需要 crop output 到特征图的大小 (或者插值特征图)
            # 简单起见，我们对 output 做 argmax，然后把它当做 label_map
            pred_logits = output[0]  # (NumClasses, D, H, W)
            pred_map = torch.argmax(pred_logits, dim=0).cpu().numpy()  # (D, H, W)

            # 这里的 pred_map 是 128x128x128 的预测结果
            # 我们将它传入 sample_features，函数内部会将其 resize 匹配特征图大小
            label_map = pred_map

            # Check if we actually have tumor pixels in this crop
            unique_classes = np.unique(label_map)
            print(f"  Case {case_id}: Found classes {unique_classes} in the cropped region.")

            # Sample Points
            X_s, y_s = sample_features(features, label_map, num_samples)
            if X_s is not None:
                global_X.append(X_s)
                global_y.extend(y_s)
        ...
        if not global_X:
            print(
                "[T-SNE] No features collected. Try increasing max_cases or check if model predicts background only.")
            return

        print(f"[T-SNE] Computing T-SNE on {len(global_X)} points...")
        X_all = torch.cat(global_X, dim=0).numpy()

        # TSNE settings
        n_samples = X_all.shape[0]
        perp = min(80, n_samples - 1) if n_samples > 1 else 1
        ...
        tsne = TSNE(
            ...
            init='pca',
            learning_rate='auto',
            early_exaggeration=12  # <--- 在这里添加，默认是12
        )
        X_emb = tsne.fit_transform(X_all)

        print("[T-SNE] Plotting...")
        df = pd.DataFrame({'x': X_emb[:, 0], 'y': X_emb[:, 1], 'Region': global_y})

        plt.figure(figsize=(12, 10))
        # 自定义颜色，确保背景是灰色，肿瘤是亮色
        custom_palette = {'Background': '#e0e0e0', 'NCR': '#ff0000', 'ED': '#00ff00', 'ET': '#0000ff'}
        # 过滤掉不在数据中的 key
        existing_keys = {k: v for k, v in custom_palette.items() if k in df['Region'].unique()}

        sns.scatterplot(data=df, x='x', y='y', hue='Region', palette=existing_keys if existing_keys else 'bright',
                        s=50, alpha=0.8)
        plt.title(f'Feature Space: {target_layer}\n(Colored by Prediction)', fontsize=16)

        out_path = join(output_folder, f'tsne_prediction_{target_layer.replace(".", "_")}.png')
...
# [ADDITION FOR T-SNE] New Entry Point
def predict_entry_point_tsne():
    import argparse
    parser = argparse.ArgumentParser(description='Run T-SNE visualization for specific layer or ALL layers.')
    ...
    parser.add_argument('-d', type=str, required=True, help='Dataset ID')
    parser.add_argument('-c', type=str, required=True, help='Configuration')
    parser.add_argument('-p', type=str, required=False, default='nnUNetPlans', help='Plans')
    parser.add_argument('-tr', type=str, required=False, default='nnUNetTrainer', help='Trainer')
    parser.add_argument('-f', nargs='+', type=str, required=False, default=(0, 1, 2, 3, 4), help='Folds')
    parser.add_argument('-chk', type=str, required=False, default='checkpoint_final.pth', help='Checkpoint')

    # T-SNE 参数
    parser.add_argument('--layer', type=str, default='all',
                        help="Layer name to visualize. Use 'all' to visualize all Encoder and Decoder stages.")
    parser.add_argument('--max_cases', type=int, default=3, help='Max cases to process')
    ...
    predictor = nnUNetPredictor(
        ...
        perform_everything_on_device=True, device=device, verbose=False, verbose_preprocessing=False
    )
    predictor.initialize_from_trained_model_folder(model_folder, args.f, args.chk)

    # === 定义要遍历的层列表 ===
    # if args.layer.lower() == 'all':
    #     # 根据 nnU-Net 6层架构定义的关键层
    #     layers_to_plot = [
    #         # --- Encoder (下采样过程) ---
    #         'encoder.stages.0',  # Stage 1 (Full Res)
    #         'encoder.stages.1',  # Stage 2
    #         'encoder.stages.2',  # Stage 3
    #         'encoder.stages.3',  # Stage 4
    #         'encoder.stages.4',  # Stage 5
    #         'encoder.stages.5',  # Stage 6 (Bottleneck, 最深层)
    #
    #         # --- Decoder (上采样过程) ---
    #         # Decoder 的索引通常是从深到浅，或者对应 Encoder 的层级
    #         'decoder.stages.0',  # Deepest Decoder Stage
    #         'decoder.stages.1',
    #         'decoder.stages.2',
    #         'decoder.stages.3',
    #         'decoder.stages.4',  # Output Stage (Final Resolution)
    #     ]
    #     print(f"\n[T-SNE] Batch mode enabled. Will visualize {len(layers_to_plot)} layers: {layers_to_plot}\n")
    # else:
    #     layers_to_plot = [args.layer]

    # === [MODIFIED] 定义要遍历的层列表 (适配 MIMS 模型) ===
    if args.layer.lower() == 'all':
        layers_to_plot = [
            # --- 新模型的 Stem (关键修改) ---
            # 'stem_mims',          # 这是整个模块的输出 (已上采样到 128x128x128)
            'stem_mims.up_sample',  # [修改] 指定 up_sample 层，配合上面的 Input 模式，抓取 64x64x64 特征

            # --- Encoder ---
            # 注意：在 MIMS 模型中，encoder.stages.0 被跳过，Hook 它不会报错但也没数据
            # 我们保留它以兼容旧 UNet 模型
            'encoder.stages.0',
            'encoder.stages.1',
            'encoder.stages.2',
            'encoder.stages.3',
            'encoder.stages.4',
            'encoder.stages.5',  # 最深层 (Bottleneck)
        ...
        print(f"\n[T-SNE] Batch mode enabled. Will visualize up to {len(layers_to_plot)} layers.\n")
    else:
        layers_to_plot = [args.layer]
    ...
    for layer_name in layers_to_plot:
        ...
        try:
            predictor.predict_and_plot_tsne(
                input_folder=args.i,
            ...
            print(f"[Skip] Layer {layer_name} not found in model. (Error: {e})")
        except Exception as e:
            print(f"[Error] Failed to process {layer_name}: {e}")
            import traceback
            traceback.print_exc()

    print("\n[T-SNE] Batch processing completed.")



if __name__ == '__main__':
    predict_entry_point_tsne()  # <--- 改为调用这个函数
    # predict_entry_point()     # <--- 注释掉原来的
