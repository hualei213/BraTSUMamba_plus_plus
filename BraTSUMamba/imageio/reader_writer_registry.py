...
import BraTSUMamba
from BraTSUMamba.imageio.natural_image_reader_writer import NaturalImage2DIO
from BraTSUMamba.imageio.nibabel_reader_writer import NibabelIO, NibabelIOWithReorient
from BraTSUMamba.imageio.simpleitk_reader_writer import SimpleITKIO
from BraTSUMamba.imageio.tif_reader_writer import Tiff3DIO
...
def determine_reader_writer_from_dataset_json(dataset_json_content: dict, example_file: str = None,
    ...
    if 'overwrite_image_reader_writer' in dataset_json_content.keys() and \
        ...
        # trying to find that class in the BraTSUMamba.imageio module
        try:
...
def recursive_find_reader_writer_by_name(rw_class_name: str) -> Type[BaseReaderWriter]:
    ret = recursive_find_python_class(join(BraTSUMamba.__path__[0], "imageio"), rw_class_name, 'BraTSUMamba.imageio')
    if ret is None:
        raise RuntimeError("Unable to find reader writer class '%s'. Please make sure this class is located in the "
                           "BraTSUMamba.imageio module." % rw_class_name)
    else:
        return ret
