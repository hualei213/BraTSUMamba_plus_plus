...
def recursive_find_resampling_fn_by_name(resampling_fn: str) -> Callable:
    ret = recursive_find_python_class(join(BraTSUMamba.__path__[0], "preprocessing", "resampling"), resampling_fn,
                                      'BraTSUMamba.preprocessing.resampling')
    if ret is None:
        raise RuntimeError("Unable to find resampling function named '%s'. Please make sure this fn is located in the "
                           "BraTSUMamba.preprocessing.resampling module." % resampling_fn)
    else:
        return ret
